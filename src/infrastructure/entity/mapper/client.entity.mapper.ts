...
@injectable()
export class ClientEntityMapper implements IEntityMapper<Client, ClientEntity> {
    public transform(item: any): any {
        if (item instanceof Client) return this.modelToModelEntity(item)
        return this.jsonToModel(item) // json
    }

    /**
     * Convert {Client} for {ClientEntity}.
     *
     ...
     * @param item
     */
    public modelToModelEntity(item: Client): ClientEntity {
        const result: ClientEntity = new ClientEntity()
    ...
    /**
     * Convert JSON for {Client}.
     *
     * @see Each attribute must be mapped only if it contains an assigned value,
     * because at some point the attribute accessed may not exist.
     * @param json
     */
    public jsonToModel(json: any): Client {
